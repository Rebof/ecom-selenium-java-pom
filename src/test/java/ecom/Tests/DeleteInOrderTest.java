...
public class DeleteInOrderTest extends BaseTest {

    String countryName = "Nepal";
    String confirmationMsg = "THANKYOU FOR THE ORDER.";
    private static final Logger logger = LogManager.getLogger(DeleteInOrderTest.class);
    ...
    public void deleteInOrderPage(HashMap<String, String> input) {
        ...
        // Navigate to order page and delete product
        productCatalogue.goToOrderFromAnywhere();
        OrderPage orderPage = new OrderPage(driver);
        orderPage.deleteProductByName(input.get("productName"));
        logger.info("Deleted product from order page: {}", input.get("productName"));

        //  Wait for toast to appear (instead of text assertion)
        orderPage.waitForVisibility(orderPage.getToastLocator(), 5);
        logger.info("Toast message appeared successfully after deletion");
    }
}
